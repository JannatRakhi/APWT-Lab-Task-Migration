...
<div class="d-flex justify-content-center align-items-center" style="min-height: 88vh;">
    <div>
        ...
        @endif
        <table class="table table-borded table-striped list-group-item-action list-group-item-info">
            ...
            @foreach($allProducts as $product)
                    <tr class="text-center">
                        ...
                        <td class="px-3">{{$product->p_name}}</td>
                        <td class="px-3">{{$product->p_category}}</td>
                        <td class="px-3">{{$product->p_code}}</td>
                        <td class="px-3">{{$product->p_desc}}</td>
                        <td class="px-3">{{$product->p_category}}</td>
                        <td class="px-3">{{$product->p_price}}</td>
                        <td class="px-3">{{$product->created_at}}</td>
                        <td class="px-3">{{$product->p_rating}}</td>
                        <td class="px-3">{{$product->p_purchased}}</td>
                       
                        <td>
                            <a class="btn  btn-success btn-sm mt-3" href={{ "editProduct/".$product->id }}>Edit</a>
                            <a class="btn btn-danger btn-sm mt-3" href={{ "deleteProduct/".$product->id }}>Delete</a>
                        </td> 
                    </tr>
            @endforeach
        </table>
        <a class="btn btn-success btn-sm mb-3" href="{{route('addProduct')}}">Add Product</a>
    </div>
</div>
@endsection
